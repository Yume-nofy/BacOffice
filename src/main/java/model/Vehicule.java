...
public class Vehicule {
    ...
    public LocalDateTime getdaterecent() {
        LocalDateTime recent = LocalDateTime.parse("11/11/11 11:11:11",
                DateTimeFormatter.ofPattern("yy/MM/dd HH:mm:ss"));
        if (reservationsAssign != null) {
            for (Reservation reservation : reservationsAssign) {
                if (reservation.getDateArrivee().isAfter(recent)) {
                    recent = reservation.getDateArrivee();
                }
            }
        }
    ...
    public LocalDateTime getdateretourAssign() {
        ...
        // System.out.println("temsp trajet = "+ tempsTrajet+" , distanceTotalValue =
        // "+distanceTotalValue+" , vitesseMoyenne = "+vitesseMoyenne);
        ...
        if (this.getDateDepart() != null) {
            if (this.getDateDepart().isAfter(dateDeparte)) {
                dateDeparte = this.getDateDepart();
            }
        }
